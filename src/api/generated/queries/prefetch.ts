...
export const prefetchUseMe = (queryClient: QueryClient, clientOptions: Options<MeData, true> = {}, options?: Omit<FetchQueryOptions<Common.MeDefaultResponse>, "queryKey" | "queryFn">) => queryClient.prefetchQuery({ queryKey: Common.UseMeKeyFn(clientOptions), queryFn: ({ signal }) => me({ ...clientOptions, signal, throwOnError: true }).then(response => response.data), ...options });
